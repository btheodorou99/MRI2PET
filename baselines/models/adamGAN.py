...
class Conv2d_kml(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
        super(Conv2d_kml, self).__init__()
        ...
        self.u_vector = nn.Parameter(torch.randn(in_channels * kernel_size * kernel_size) * 1e-7)
...
class ConvTranspose2d_kml(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True):
        super(ConvTranspose2d_kml, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        ...
        self.W = nn.Parameter(torch.randn(in_channels, out_channels, kernel_size, kernel_size))
        if bias:
            self.bias = nn.Parameter(torch.zeros(out_channels))
        else:
            self.register_parameter('bias', None)
        ...
        self.v_vector = nn.Parameter(torch.randn(out_channels * kernel_size * kernel_size) * 1e-7)
        if bias:
            self.b_vector = nn.Parameter(torch.zeros(self.out_channels))
        else:
            self.register_parameter('b_vector', None)
    ...
    def forward(self, input):
        ...
        weight = self.W * (torch.ones_like(self.W) + W_hat)
        bias   = self.bias + self.b_vector
        out = F.conv_transpose2d(input, weight, bias=bias, stride=self.stride, padding=self.padding)
        return out
...
class ImageEncoder(nn.Module):
    def __init__(self, config, is_mri=True):
        ...
        self.conv2 = Conv2d_kml(32, 64, kernel_size=3, stride=1, padding=1)
        self.conv3 = Conv2d_kml(64, 128, kernel_size=3, stride=1, padding=1)
        self.conv4 = Conv2d_kml(128, 256, kernel_size=3, stride=1, padding=1)
...
class Generator(nn.Module):
    def __init__(self, config):
        ...
        self.init_map = Linear_kml(config.z_dim + config.embed_dim, 32 * (self.init_dim ** 2))
    ...
    def _gen_block(self, in_channels, out_channels):
        return nn.Sequential(
            ...
            ConvTranspose2d_kml(in_channels, out_channels, kernel_size=4, stride=2, padding=1, bias=False),
