...
from ...models.ganModel import ImageEncoder
import random
...
class Generator(nn.Module):
    def __init__(self, config):
        ...
        self.init = InitLayer(nz+config.embed_dim, channel=8)
                                
        self.feat_8   = UpBlockComp(8, 16)
        self.feat_16  = UpBlock(16, 32)
        self.feat_32  = UpBlockComp(32, 32)
        self.feat_64  = UpBlock(32, 32)
        self.feat_128 = UpBlockComp(32, 16)  
        self.feat_256 = UpBlock(16, 8) 

        self.se_64  = SEBlock(8, 32)
        self.se_128 = SEBlock(16, 16)
        self.se_256 = SEBlock(8, 8)

        self.to_128 = conv3d(16, nc, 1, 1, 0, bias=False) 
        self.to_big = conv3d(8, nc, 3, 1, 1, bias=False) 
        
        if im_size > 256:
            self.feat_512 = UpBlockComp(8, 8) 
            self.se_512 = SEBlock(8, 8)
        if im_size > 512:
            self.feat_1024 = UpBlock(8, 8)  
...
class Discriminator(nn.Module):
    ...
    def forward(self, imgs, context, label, part=None):
        if type(imgs) is not list:
            imgs = [F.interpolate(imgs, size=self.im_size), F.interpolate(imgs, size=128)]
            
        context = [F.interpolate(context, size=self.im_size), F.interpolate(context, size=128)]
        
        imgs = [torch.cat([imgs[0], context[0]], dim=1), torch.cat([imgs[1], context[1]], dim=1)]
