...
model = inception_v3(aux_logits=False).to(device)
model = nn.Sequential(*list(model.children())[:20])
state_dict = torch.load("./src/data/radimagenet_inception.pt")
...
def get_inception_features(model, dataset, idx_mapping):
    ...
    with torch.no_grad():
        for i in range(0, len(dataset), BATCH_SIZE):
            ...
            output = model(batch_images).squeeze(-1).squeeze(-1)
...
for k in tqdm(model_keys):
    ...
    pickle.dump(fid_value, open(f'./src/results/quantitative_evaluations/{k}_content_med_fid.pkl', 'wb'))
