...
train_dataset = [pet_path for (mri_path, pet_path) in train_dataset]
train_dataset = random.sample(train_dataset, len(test_dataset))
train_act = get_inception_features(model, train_dataset)

fid_values = []
for i in range(config.n_bootstrap):
    bs_indices = resample(np.arange(len(train_act)), replace=True)
    bs_real = test_act[bs_indices]
    bs_fake = train_act[bs_indices]
    fid_value = calculate_fid(bs_real, bs_fake)
    fid_values.append(fid_value)
fid_value = (np.mean(fid_values), np.std(fid_values) / np.sqrt(config.n_bootstrap))
print(f'Real FID: {fid_value[0]:.3f} \\pm {fid_value[1]:.3f}')
pickle.dump(fid_value, open(f'./src/results/quantitative_evaluations/real_fid.pkl', 'wb'))
