...
def train_model(train_data, has_mri, has_pet, key, seed):
    ...
    for e in tqdm(range(config.downstream_epoch), desc=f"Training {key} Model", leave=False):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            curr_patience = 0
            torch.save(model.state_dict(), f'./src/save/downstream_mmse_highLR_{key}.pt')
        else:
            curr_patience += 1
    ...
    model.load_state_dict(torch.load(f'./src/save/downstream_mmse_highLR_{key}.pt'))
    return model
...
for key, hasMRI, hasPET, data in tqdm(experiments):
    ...
    pickle.dump(metrics_dict, open(f'./src/results/quantitative_evaluations/utility_mmse_highLR_{key}.pkl', 'wb'))
...
model.load_state_dict(torch.load(f'./src/save/downstream_mmse_highLR_RealPET.pt'))
...
model.load_state_dict(torch.load(f'./src/save/downstream_mmse_highLR_RealPaired.pt'))
...
plt.tight_layout()
plt.savefig('./src/results/quantitative_evaluations/mmse_highLR_pet_correlation.png')
plt.close()
...
plt.tight_layout()
plt.savefig('./src/results/quantitative_evaluations/mmse_highLR_paired_correlation.png')
plt.close()
...
pickle.dump(comparison_results, open('./src/results/quantitative_evaluations/mmse_highLR_real_vs_synthetic_comparison.pkl', 'wb'))
