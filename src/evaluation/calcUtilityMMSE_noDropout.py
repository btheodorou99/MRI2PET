...
def train_model(train_data, has_mri, has_pet, key, seed):
    ...
    model = ImageRegressor(config, has_mri, has_pet, has_dropout=False).to(device)
    ...
    for e in tqdm(range(config.downstream_epoch), desc=f"Training {key} Model", leave=False):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            curr_patience = 0
            torch.save(model.state_dict(), f'./src/save/downstream_mmse_noDropout_{key}.pt')
        else:
            curr_patience += 1
    ...
    model.load_state_dict(torch.load(f'./src/save/downstream_mmse_noDropout_{key}.pt'))
    return model
...
for key, hasMRI, hasPET, data in tqdm(experiments):
    ...
    pickle.dump(metrics_dict, open(f'./src/results/quantitative_evaluations/utility_mmse_noDropout_{key}.pkl', 'wb'))
...
model = ImageRegressor(config, False, True, has_dropout=False).to(device)
model.load_state_dict(torch.load(f'./src/save/downstream_mmse_noDropout_RealPET.pt'))
...
model = ImageRegressor(config, True, True, has_dropout=False).to(device)
model.load_state_dict(torch.load(f'./src/save/downstream_mmse_noDropout_RealPaired.pt'))
...
plt.tight_layout()
plt.savefig('./src/results/quantitative_evaluations/mmse_noDropout_pet_correlation.png')
plt.close()
...
plt.tight_layout()
plt.savefig('./src/results/quantitative_evaluations/mmse_noDropout_paired_correlation.png')
plt.close()
...
pickle.dump(comparison_results, open('./src/results/quantitative_evaluations/mmse_noDropout_real_vs_synthetic_comparison.pkl', 'wb'))
