...
adni_scores['MMSCORE'] = (adni_scores['MMSCORE'] - np.mean(adni_scores['MMSCORE'])) / np.std(adni_scores['MMSCORE'])
...
def train_model(train_data, has_mri, has_pet, key, seed):
    ...
    for e in tqdm(range(config.downstream_epoch), desc=f"Training {key} Model", leave=False):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            curr_patience = 0
            torch.save(model.state_dict(), f'./src/save/downstream_mmse_normalizedScores_{key}.pt')
        else:
            curr_patience += 1
    ...
    model.load_state_dict(torch.load(f'./src/save/downstream_mmse_normalizedScores_{key}.pt'))
    return model
...
model.load_state_dict(torch.load(f'./src/save/downstream_mmse_normalizedScores_RealPET.pt'))
...
model.load_state_dict(torch.load(f'./src/save/downstream_mmse_normalizedScores_RealPaired.pt'))
...
plt.tight_layout()
plt.savefig('./src/results/quantitative_evaluations/mmse_normalizedScores_pet_correlation.png')
plt.close()
...
plt.tight_layout()
plt.savefig('./src/results/quantitative_evaluations/mmse_normalizedScores_paired_correlation.png')
plt.close()
...
pickle.dump(comparison_results, open('./src/results/quantitative_evaluations/mmse_normalizedScores_real_vs_synthetic_comparison.pkl', 'wb'))
