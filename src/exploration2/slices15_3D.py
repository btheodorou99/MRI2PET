...
def get_batch(dataset, loc, batch_size):
    ...
    for i, (m, p) in enumerate(image_paths):
        ...
        batch_image[i] = load_image(p, is_mri=False)[22:37, :, :]
...
# if os.path.exists(f"./src/save/baseDiffusion3D.pt"):
#     print("Loading previous model")
#     checkpoint = torch.load(f'./src/save/baseDiffusion3D.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
        ...
        # }
        # torch.save(state, f'./src/save/baseDiffusion3D.pt')