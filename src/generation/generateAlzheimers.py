...
os.makedirs("./src/results/alzheimers_case_study", exist_ok=True)
for i in tqdm(range(0, len(test_dataset), config.batch_size)):
    ...
    for j in range(sample_contexts.size(0)):
        ...
        save_slice_plots(real_mri, real_pet, generated_pet, f"./src/results/alzheimers_case_study/", f"{subject}_{date}")
