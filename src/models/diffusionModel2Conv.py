...
class DiffusionModel(nn.Module):
    def __init__(self, config, noise=False):
        ...
        self.outc_initial = nn.Conv2d(16, config.n_pet_channels, kernel_size=1)

        # Refinement block (optional, can be a series of convolutional layers)
        self.refinement = nn.Sequential(
            nn.Conv2d(config.n_pet_channels, config.n_pet_channels, kernel_size=3, padding=1),
        ...
        # Final output layer
        self.outc_final = nn.Conv2d(config.n_pet_channels, config.n_pet_channels, kernel_size=1)
