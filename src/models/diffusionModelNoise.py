...
class DiffusionModel(nn.Module):
    ...
    def noise_images(self, x, t, noiseLevel=1):
    ...
    def forward(self, context, input_images, gen_loss=True, noiseLevel=1):
        noiseLevel = torch.sqrt(torch.tensor(noiseLevel))
        t = self.sample_timesteps(input_images.size(0), context.device)
        noised_images, noise = self.noise_images(input_images, t, noiseLevel)
