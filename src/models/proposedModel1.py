# Calculate MSE and Forward Generation Using MRI
...
class DiffusionModel(nn.Module):
    ...
    def forward(self, context, input_images, gen_loss=True):
        ...
        if gen_loss:
            noiseFromContext = input_images - context
            loss = F.mse_loss(noise + noiseFromContext, predictedNoise)
    ...
    def generate(self, context):
        ...
        for timestep in tqdm(reversed(range(1, self.num_timesteps)), total=self.num_timesteps-1):
            ...
            predicted_total_noise = self._forward(x, t, context) # pred_noise + pred_diff = pred_noise + (pred_input - context)
            # sqrt_alpha_hat * input + sqrt_one_minus_alpha_hat * Ɛ = x
            predicted_noise_plus_predicted_input = predicted_total_noise + context # From noising equation: input = (x - sqrt_one_minus_alpha_hat * Ɛ) / sqrt_alpha_hat => So this is: (x - sqrt_one_minus_alpha_hat * Ɛ) / sqrt_alpha_hat + Ɛ where Ɛ is the predicted noise
            ...
            predicted_noise = ((torch.sqrt(alpha_hat) * predicted_noise_plus_predicted_input) - x) / (torch.sqrt(alpha_hat) - torch.sqrt(1 - alpha_hat))
            if timestep > 1:
