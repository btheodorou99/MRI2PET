# Gabor Filter Loss to Emphasize Details, With MRI Component
...
class DiffusionModel(nn.Module):
    ...
    def gabor_filter_loss(img1, img2, kernel_size=15, sigma=3.0, theta=1.0, lamda=10.0, gamma=0.5):
        ...
        filtered_img2 = F.conv2d(img2, gabor_kernel, padding=kernel_size//2)

        # Calculate loss
        loss = F.mse_loss(filtered_img1, filtered_img2)
    ...
    def forward(self, context, input_images, gen_loss=True, weight=0.0):
        ...
        if gen_loss:
            ...
            gabor_loss = self.gabor_filter_loss(input_images, predictedImage)
            gabor_mri_loss = self.gabor_filter_loss(context, predictedImage)
            return loss + self.lambda1 * gabor_loss + weight * gabor_mri_loss, predictedNoise
