# Gradient Magnitude Similarity Loss to Emphasize Details, With MRI Component
...
class DiffusionModel(nn.Module):
    ...
    def gradient_magnitude_similarity_loss(img1, img2):
        ...
        grad_mag_img1 = gradient_magnitude(img1)
        grad_mag_img2 = gradient_magnitude(img2)

        loss = F.mse_loss(grad_mag_img1, grad_mag_img2)
    ...
    def forward(self, context, input_images, gen_loss=True, weight=0.0):
        ...
        if gen_loss:
            ...
            gradient_loss = self.gradient_magnitude_similarity_loss(input_images, predictedImage)
            gradient_mri_loss = self.gradient_magnitude_similarity_loss(context, predictedImage)
            return loss + self.lambda1 * gradient_loss + weight * gradient_mri_loss, predictedNoise
