# Using MRI-PET pair classification model as loss component to push generated PET images to be like the MRI context
...
class DiffusionModel(nn.Module):
    ...
    def forward(self, context, input_images, gen_loss=True, pairModel=None):
        ...
        if gen_loss:
            ...
            if pairModel is not None:
                predictedImage = self.construct_image(noised_images, t, predictedNoise)
                pairLoss = F.binary_cross_entropy(pairModel(context, predictedImage), torch.ones_like(t).float().to(context.device))
                loss += self.lambda1 * pairLoss
