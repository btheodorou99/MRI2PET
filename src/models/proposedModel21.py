# Using MRI or PET disease classification model as loss component to push generated PET images to be similar to real PET and context MRI based on intermediate features
    ...
    def forward(self, context, input_images, gen_loss=True, clsModel=None):
        ...
        if gen_loss:
            ...
            if clsModel is not None:
                ...
                trueFeatures = clsModel(input_images, intermed=True)
                mriFeatures = clsModel(context, intermed=True)
                pairLoss = self.lambda1 * self.compute_pairwise_sim_loss(predFeatures, trueFeatures) + self.lambda2 * self.compute_pairwise_sim_loss(predFeatures, mriFeatures)
                loss += pairLoss
