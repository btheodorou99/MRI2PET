# Using MRI or PET disease classification model to generate intermediate features to prevent overfitting by keeping distribution of features similar to generations by the source (not fine-tuned) model
...
class ImageClassifier(nn.Module):
    def __init__(self, config, output_dim):
        ...
        self.enc = ImageEncoder(config, isMri=False)
        self.fc1 = nn.Linear(config.embed_dim, 256)
        self.fc2 = nn.Linear(256, 128)
        self.fc3 = nn.Linear(128, output_dim)
        
    def forward(self, image, intermed=False):
        enc = self.enc(image)
...
class DiffusionModel(nn.Module):
    ...
    def forward(self, context, input_images, gen_loss=True, clsModel=None, outputImage=False, sourceImage=None):
        ...
        if gen_loss:
            ...
            if clsModel is not None:
                predictedImage = self.construct_image(noised_images, t, predictedNoise)
                predFeatures = clsModel(predictedImage, intermed=True)
                sourceFeatures = clsModel(sourceImage, intermed=True)
                pairLoss = self.lambda1 * self.compute_pairwise_sim_loss(predFeatures, sourceFeatures)
                loss += pairLoss
