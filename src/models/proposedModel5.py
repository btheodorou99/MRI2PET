# Adaptive Frequency Loss Beginning With Emphasis on Low Frequency Features and Ending With Emphasis on High Frequency Features With MRI and PET Images
...
class DiffusionModel(nn.Module):
    ...
    def compute_high_freq_loss(self, x_T, x_R, x_C, weights):
        ...
        LL = torch.ger(L, L).view(1, 1, 2, 2).repeat(1, self.n_channels, 1, 1).to(x_T.device)
        LH = torch.ger(L, H).view(1, 1, 2, 2).repeat(1, self.n_channels, 1, 1).to(x_T.device)
        HL = torch.ger(H, L).view(1, 1, 2, 2).repeat(1, self.n_channels, 1, 1).to(x_T.device)
        HH = torch.ger(H, H).view(1, 1, 2, 2).repeat(1, self.n_channels, 1, 1).to(x_T.device)
        ...
        hf_R = lh_R + hl_R + hh_R
        hf_R = hf_R.flatten(1)
        ll_C = F.conv2d(x_C, LL, stride=2)
        lh_C = F.conv2d(x_C, LH, stride=2)
        hl_C = F.conv2d(x_C, HL, stride=2)
        hh_C = F.conv2d(x_C, HH, stride=2)
        lf_C = ll_C + lh_C + hl_C
        lf_C = lf_C.flatten(1)
        hf_C = lh_C + hl_C + hh_C
        hf_C = hf_C.flatten(1)
        
        lf_mse_loss = F.mse_loss(lf_T, lf_R)
        hf_mse_loss = F.mse_loss(hf_T, hf_R)
        
        lf_mse_loss_C = F.mse_loss(lf_T, lf_C)
        hf_mse_loss_C = F.mse_loss(hf_T, hf_C)
        
        return lf_weight_pet * lf_mse_loss + hf_weight_pet * hf_mse_loss + lf_weight_mri * lf_mse_loss_C + hf_weight_mri * hf_mse_loss_C
