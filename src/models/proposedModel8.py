# Aiding Structural Similarity Betwen MRI and PET using SSIM (Structual Similarity Index) Loss Component
...
class DiffusionModel(nn.Module):
    ...
    def create_window(self, window_size, sigma, channels):
        _1D_window = self.gaussian_window(window_size, sigma).unsqueeze(1)
        _2D_window = _1D_window.mm(_1D_window.t()).float().unsqueeze(0).unsqueeze(0)
        window = _2D_window.expand(channels, 1, window_size, window_size).contiguous()
        return window

    def ssim(self, img1, img2, window_size=16, size_average=True):
        img1 = (img1 + 1) / 2
        img2 = (img2 + 1) / 2
        C1 = 0.01 ** 2
        C2 = 0.03 ** 2
        channels = img1.size(1)
        window = self.create_window(window_size, window_size / 3, channels).to(img1.device)
    ...
    def forward(self, context, input_images, gen_loss=True, weight=0.0):
        ...
        if gen_loss:
            ...
            mriLoss = 1 - self.ssim(context, predictedImage)
            return loss + weight * mriLoss, predictedNoise
