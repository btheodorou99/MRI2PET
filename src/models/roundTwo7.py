# Diffusion Cycle like CycleGAN Back to MRI to ensure the MRI features are between included in the PET Scan
...
class DiffusionModel(nn.Module):
    ...
    def forward(self, context, input_images, gen_loss=True, cycle=0, weight=0.0):
        ...
        if gen_loss:
            ...
            if cycle > 0:
                if cycle == 1:
                    predictedMRI = self._backward(input_images)
                elif cycle == 2:
                    predictedMRI = self._backward(self.construct_image(noised_images, t, predictedNoise))
                cycleLoss = F.mse_loss(context, predictedMRI)
                loss += self.lambda1 * cycleLoss
            ...
            return loss + mriLoss + (self.lambda2 * laplace_loss), predictedNoise
