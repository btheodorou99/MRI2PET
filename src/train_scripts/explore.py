...
config.n_pet_channels = 20

train_dataset = pickle.load(open('./src/data/trainDataset.pkl', 'rb'))[:500]
val_dataset = pickle.load(open('./src/data/valDataset.pkl', 'rb'))[:100]
...
def get_batch(dataset, loc, batch_size):
    ...
    for i, (m, p) in enumerate(image_paths):
        ...
        batch_image[i] = load_image(p, is_mri=False)[20:40, :, :]
...
if os.path.exists(f"./src/save/exploration.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/exploration.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
        ...
        torch.save(state, f'./src/save/exploration.pt')