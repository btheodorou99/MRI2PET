...
if os.path.exists("./src/data/globalMeanPretrainStyle.pkl"):
    ...
    pickle.dump(global_mean, open("./src/data/globalMeanPretrainStyle.pkl", "wb"))
    pickle.dump(global_std, open("./src/data/globalStdPretrainStyle.pkl", "wb"))
...
if os.path.exists(f"./src/save/mri2pet_base_pScale.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/mri2pet_base_pScale.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'./src/save/mri2pet_base_pScale.pt')