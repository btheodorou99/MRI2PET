...
from ..models.diffusionModelBig import DiffusionModel
...
if os.path.exists(f"./src/save/baseDiffusionBig.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/baseDiffusionBig.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
        ...
        torch.save(state, f'./src/save/baseDiffusionBig.pt')