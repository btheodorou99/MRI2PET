...
from ..models.diffusionModelNoise import DiffusionModel
...
if os.path.exists(f"./src/save/baseDiffusionNoiseRampup.pt"):
    ...
    checkpoint = torch.load(f'./src/save/baseDiffusionNoiseRampup.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
        ...
        torch.save(state, f'./src/save/baseDiffusionNoiseRampup.pt')