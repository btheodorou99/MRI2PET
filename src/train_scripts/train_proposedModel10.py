...
if os.path.exists(f"./src/save/proposedModel10.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/proposedModel10.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'./src/save/proposedModel10.pt')
