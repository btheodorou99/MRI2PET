...
if os.path.exists(f"./src/save/proposedModel21.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/proposedModel21.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'./src/save/proposedModel21.pt')
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    if cur_cls_val_loss < global_loss:
        ...
        torch.save(state, f'./src/save/proposedModel21_cls.pt')
        
cls_model.load_state_dict(torch.load(f'./src/save/proposedModel21_cls.pt')['model'])
cls_model.eval()
