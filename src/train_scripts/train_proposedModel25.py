...
from ..models.proposedModel25 import DiffusionModel
...
if os.path.exists(f"./src/save/proposedModel25.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/proposedModel25.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'./src/save/proposedModel25.pt')
...
for e in tqdm(range(config.epoch)):
    ...
    for i in range(0, len(train_dataset), config.batch_size):
        ...
        optimizer.zero_grad()
        loss, _ = model(batch_context, batch_images, gen_loss=True, cycle=1 if e < BACKWARDS_PRETRAIN_EPOCH else 2)
        loss.backward()
        optimizer.step()
