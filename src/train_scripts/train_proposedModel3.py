...
from ..models.proposedModel3 import DiffusionModel
...
model = DiffusionModel(config, mean_mri).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=config.lr)
if os.path.exists(f"./src/save/proposedModel3.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/proposedModel3.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
    ...
    torch.save(state, f'./src/save/proposedModel3.pt')
