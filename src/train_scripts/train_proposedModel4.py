...
from ..models.proposedModel4 import DiffusionModel
...
FREQ_SCHEDULE = list(zip(np.linspace(0.5, 0, config.epoch), np.linspace(0, 0.5, config.epoch)))
...
if os.path.exists(f"./src/save/proposedModel4.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/proposedModel4.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'./src/save/proposedModel4.pt')
