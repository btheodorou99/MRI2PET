...
if os.path.exists(f"./src/save/proposedModel5.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/proposedModel5.pt', map_location=torch.device(device))
...
#         'mode': 'pretrain'
#     }
#     torch.save(state, f'./src/save/proposedModel5.pt')
...
for e in tqdm(range(config.epoch)):
    ...
    for i in range(0, len(train_dataset), config.batch_size):
        ...
        freq_loss = model.compute_high_freq_loss(x_T, batch_images, batch_context, FREQ_SCHEDULE[e])
        loss = loss + freq_loss
