...
from ..models.roundTwo2 import DiffusionModel
...
MRI_LOSS_SCHEDULE = [max(0.5 - (0.5 * e / (config.epoch // 10)), 0) for e in range(config.epoch)]
...
if os.path.exists(f"./src/save/roundTwo2.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/roundTwo2.pt', map_location=torch.device(device))
...
#         'mode': 'pretrain'
#     }
#     torch.save(state, f'./src/save/roundTwo2.pt')
