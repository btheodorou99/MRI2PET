...
from ..models.roundTwo7 import DiffusionModel
...
MRI_LOSS_SCHEDULE = [max(0.5 - (0.5 * e / (config.epoch // 4)), 0) for e in range(config.epoch)]
...
if os.path.exists(f"./src/save/roundTwo7.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/roundTwo7.pt', map_location=torch.device(device))
...
#         'mode': 'pretrain'
#     }
#     torch.save(state, f'./src/save/roundTwo7.pt')
...
for e in tqdm(range(config.epoch)):
    ...
    for i in range(0, len(train_dataset), config.batch_size):
        ...
        loss, _ = model(batch_context, batch_images, gen_loss=True, cycle=1 if e < BACKWARDS_PRETRAIN_EPOCH else 2, weight=MRI_LOSS_SCHEDULE[e])
