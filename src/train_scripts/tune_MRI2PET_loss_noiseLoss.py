...
if os.path.exists(f"./src/save/mri2pet_loss.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'./src/save/mri2pet_loss.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
        ...
        torch.save(state, f'./src/save/mri2pet_loss_noiseLoss.pt')