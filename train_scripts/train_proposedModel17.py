...
if os.path.exists(f"../save/proposedModel17.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'../save/proposedModel17.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'../save/proposedModel17.pt')
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    if cur_cls_val_loss < global_loss:
        ...
        torch.save(state, f'../save/proposedModel17_cls.pt')
        
cls_model.load_state_dict(torch.load(f'../save/proposedModel17_cls.pt')['model'])
cls_model.eval()
