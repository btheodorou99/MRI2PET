...
from ..models.proposedModel24 import DiffusionModel, ImagePairClassifier

SEED = 4
cudaNum = 0
NUM_SAMPLES = 25
LABEL_DIR = '../data/patient_labels'
random.seed(SEED)
np.random.seed(SEED)
...
def get_batch(dataset, loc, batch_size):
    ...
    bs = len(image_paths)
    batch_context = torch.zeros(bs, config.n_channels, config.pet_image_dim, config.pet_image_dim, dtype=torch.float, device=device)
...
if os.path.exists(f"../save/proposedModel24.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'../save/proposedModel24.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'../save/proposedModel24.pt')
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    if cur_cls_val_loss < global_loss:
        ...
        torch.save(state, f'../save/proposedModel24_cls.pt')
        
cls_model.load_state_dict(torch.load(f'../save/proposedModel24_cls.pt')['model'])
cls_model.eval()
...
for e in tqdm(range(config.epoch)):
    ...
    for i in range(0, len(train_dataset), config.batch_size):
        ...
        optimizer.zero_grad()
        predImage_S = model_S(batch_context, batch_images, outputImage=True)
        loss, _ = model(batch_context, batch_images, gen_loss=True, pairModel=cls_model, sourceImage=predImage_S)
        loss.backward()
        optimizer.step()
