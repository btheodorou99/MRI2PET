...
if os.path.exists(f"../save/proposedModel4.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'../save/proposedModel4.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'../save/proposedModel4.pt')
