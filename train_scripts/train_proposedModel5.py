...
from ..models.proposedModel5 import DiffusionModel
...
FREQ_SCHEDULE = [(
                    0 if e >= config.epoch // 2 else 0.5 * e / (config.epoch // 2),
                    0 if e >= config.epoch // 2 else 0.5 - 0.5 * e / (config.epoch // 2),
                    0.5 * e / config.epoch,
...
if os.path.exists(f"../save/proposedModel5.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'../save/proposedModel5.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'../save/proposedModel5.pt')
