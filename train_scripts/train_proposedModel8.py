...
from ..models.proposedModel8 import DiffusionModel
...
if os.path.exists(f"../save/proposedModel8.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'../save/proposedModel8.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'../save/proposedModel8.pt')
...
for e in tqdm(range(config.epoch)):
    ...
    for i in range(0, len(train_dataset), config.batch_size):
        ...
        optimizer.zero_grad()
        loss, _ = model(batch_context, batch_images, gen_loss=True, weight=1.0 if e < 10 else 0.1)
        loss.backward()
        optimizer.step()
