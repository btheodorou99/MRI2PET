...
if os.path.exists(f"../save/proposedModel9.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'../save/proposedModel9.pt', map_location=torch.device(device))
...
for e in tqdm(range(config.pretrain_epoch)):
    ...
    torch.save(state, f'../save/proposedModel9.pt')
